...
//         $editor[0].editor.derivation.save($editor[0].editor.extraparams);
    ...
    $element.find('.mathquill-editable').focusout(function(e){
        ...
        var location = jQuery(this).parents('td').attr('loc');
        var edittype = jQuery(this).parents('td').attr('class');
        var derivation = $editor[0].editor.derivation;
        var changedPos = derivation.findLocation(location);
        
        QedEditor.buttonpanel.lastfocus = jQuery(this);
        jQuery(this).parents('tr:eq(0)').removeClass('sdeditfocuson');
        var newtext = jQuery(this).mathquill('latex').replace(/</g,'\\lt ').replace(/>/g,'\\gt ');
        var oldtext = derivation.findLocation(location).text;
        if (oldtext != newtext){
            if (typeof(changedPos.virgin) != 'undefined' && changedPos.virgin){
                jQuery(this).parents('td.virgin').removeClass('virgin');
            }
            jQuery(this).parents('.qededitor').eq(0).find('a.command_qedundo').addClass('active');
            jQuery(this).parents('.qededitor').eq(0).find('a.command_qedredo').removeClass('active');
...
//             Leave this for testing of new key commands.
    ...
    $element.find('.sdbeforebuttons').mouseenter(function(e){
        var editor = jQuery(this).parents('.qededitor')[0].editor;
        var $tdelement = jQuery(this).parents('td').eq(0);
        jQuery('#qededitormenuwrapper').remove();
...
/*                        'undo': !editor.undoStack.isempty(),
    ...
    $element.find('.sdafterbuttons').mouseenter(function(e){
        var editor = jQuery(this).parents('.qededitor')[0].editor;
        var $tdelement = jQuery(this).parents('td').eq(0);
        jQuery('#qededitormenuwrapper').remove();
...
//                 savetiddler = this.extraparams.tiddler;
            ...
            var i = 0;
            while (this.derivationExists(derivName+'sub'+i)){
                derivations[derivName+'sub'+i] = this.getTiddlyDeriv(derivName+'sub'+i, savetiddler);
                i++;
            }
            break;
        case 'moodlesa':
            ...
            var i = 0;
            while (this.derivationExists(derivName+'sub'+i)){
                derivations[derivName+'sub'+i] = this.getMoodleSADeriv(derivName+'sub'+i);
                i++;
            }
            break;
        case 'moodledesc':
            ...
            var i = 0;
            while (this.derivationExists(derivName+'sub'+i)){
                derivations[derivName+'sub'+i] = this.getMoodleDescDeriv(derivName+'sub'+i);
                i++;
            }
            break;
        default:
...
//        beforebuttons = '<a href="javascript:;" class="removedermot sdremovebutton"><span>{-}</span></a>';
        ...
        minusbuttons = '<span class="minusbuttons" menuitems="'+minusbuttons.join(' ')+'"></span>';
        beforebuttons = '<div class="sdbeforebuttons">'+minusbuttons+beforebuttons+'</div>';
        tdBefore = '<tr><td>'+beforebuttons+'⊩</td>';
